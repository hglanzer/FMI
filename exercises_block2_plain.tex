...
In the next step we create a Table and to Translate the “labeling formulas” to clauses:
    ...
    \begin{figure}[ht]
      \centering
      \begin{tikzpicture}
        \node[label={[red]above:$l_4$}] {$\ra$}
        child {
          node {$a$}
        }
        child {
          node[label={[red]right:$l_3$}] {$\lor$}
          child {
            node {$b$}
          }
          child {
        ...
        ;
      \end{tikzpicture}
      \caption{Formula tree for the corresponding formula (the need labels are colored red)}
      \label{fig1}
    \end{figure}

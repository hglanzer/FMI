...
\Lb
	...
	{\CA{F \wedge (x\leq 0)}{while\ x \neq y\ do\ x:=x+1;y:=y+2\ od}{G}}
      ...
      {
	(F\wedge x\geq 0)\Rightarrow Inv
      }
      {
	\WH
	{
	...
	}
	{
	  ...
	  }
	  {
	    ...
	    }
	    {
	    	...
	    	{
	    		\CA{Inv\wedge x+1\neq y\wedge t=t_0[y|y+2]}{y:=y+2}{\overbrace{Inv\wedge x\neq y+1\wedge t=t_0}^{B}}
	    	}
	    }
	    {B\Rightarrow (Inv\wedge 0\leq t<t_0)}
	  }
	}
      }
$
...
\newline
\begin{itemize}
	\item	$x=2y\wedge y>2\Rightarrow I$\\
				$x=2y\wedge y>2\Rightarrow F[x|x+y]$\\
				$x=2y\wedge y>2\Rightarrow x+y=2y\wedge y>2$\\
				$x=2y\wedge y>2\Rightarrow x=y\wedge y>2$
	\item	$F\wedge x<0\Rightarrow False$\\
				$x=y\wedge y>2\wedge x<0\Rightarrow False$ 
	\item	$F\wedge x\geq 0\Rightarrow Inv(=TRUE\ by\ Assumption)$\\
				$x=y\wedge y\geq 2\wedge x\geq 0\Rightarrow TRUE$
	\item	$Boundfunction\ t=y-x$\\
\end{itemize}
...
\end{itemize}
${ F_i } = { e ^ wp(p, F_{iÃ¢ÂÂ1}) }$ . . . set of states such that
\begin{itemize}
\item p is executed once (because e is true), resulting in a state where
\item i Ã¢ÂÂ 1 further iterations will lead to a G-state. 
\end{itemize}
